...
//
// Mux also takes care of sanitizing the URL request path and the Host
...
func (mux *Mux) Handler(r *http.Request) (h http.Handler, pattern string) {
	...
	if r.Method == "CONNECT" {
		...
		// but the p canonicalization does not.
	...
	// All other requests have any port stripped and p cleaned
	// before passing to mux.handler.
	host := stripHostPort(r.Host)
	p := cleanPath(r.URL.Path)

	// If the given p is /tree and its handler is not registered,
	// redirect for /tree/.
	if u, ok := mux.redirectToPathSlash(host, p, r.URL); ok {
		return http.RedirectHandler(u.String(), http.StatusMovedPermanently), u.Path
	}

	if p != r.URL.Path {
		_, pattern = mux.handler(host, p)
...
// If a handler already exists for pattern, itw ill be overwritten.
